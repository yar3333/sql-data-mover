...
- **Уникальное поле** для каждой таблицы — по нему строки источника сопоставляются с уже
  существующими строками приёмника (может не совпадать с первичным ключом):
...
3. **Поля и копирование** — для каждой таблицы выберите уникальное поле сопоставления
   (по умолчанию — первичный ключ, если он один) и нажмите «Запустить копирование».
...
2. Для каждой таблицы в приёмнике загружается словарь «значение уникального поля → целевой ID»
   (целевой ID = identity, иначе первичный ключ, иначе само уникальное поле).
...
обновление по уникальному полю, транзакции) и зарегистрируйте его в `DbProviderFactory`.
...
- Уникальное поле должно быть действительно уникальным в источнике (иначе «последняя строка
  побеждает»).
