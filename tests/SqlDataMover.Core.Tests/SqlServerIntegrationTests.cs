...
public class SqlServerIntegrationTests
{
    ...
    public async Task Copies_dependent_tables_with_composite_match_and_identity_remap()
    {
        ...
        await VerifyIdentitySeedsAsync(verify);
    }
    ...
    private static async Task VerifyIdentitySeedsAsync(SqlConnection dest)
    {
        // Таблицы, копируемые по identity-полю, после синхронизации имеют счётчик,
        // выровненный по источнику: следующие autogenerated ID совпадут.
        await using var src = new SqlConnection(SourceConnectionString);
        await src.OpenAsync();

        foreach (var table in new[] { "tests.Orders", "tests.OrderItems", "tests.Employees" })
        {
            var srcSeed = await IdentityCurrentAsync(src, table);
            var destSeed = await IdentityCurrentAsync(dest, table);
            Assert.Equal(srcSeed, destSeed);
        }
    }

    private static async Task<long> IdentityCurrentAsync(SqlConnection connection, string table)
    {
        await using var cmd = new SqlCommand($"SELECT IDENT_CURRENT('{table}')", connection);
